...
# Workshop material - Day #1 - Git/VS Code
  * Setting up git
    ...
    * Open https://github.com/HexaInnovLab/ftp38, and look at the WEEK1.md
  ...
  * Git (setup/checkout/clone/pull/push)
    ...
    * `git config --global -l`
    * Open https://github.com/HexaInnovLab/ftp38
      * look at source code organization
    * Go back to gitbash and clone the project
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone git@github.com:HexaInnovLab/ftp38.git`
    * `cd ftp38`
    * `git status`
  * open Visual Studio Code
    * Open folder c:\users\Hvuser\workspace\ftp38
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP38;` and click the lightning button
  * `CREATE USER 'FTP38'@'localhost' IDENTIFIED BY 'FTP38';`
  * `GRANT ALL ON FTP38.* TO 'FTP38'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp38
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.ftp38.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp38-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp38.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp38/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp38/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp38-0.0.1-SNAPSHOT.war ./ftp38.war`
   * `cd dist`
   * `jar -uvf ../ftp38.war *`
   * `cd ..`
   * `jar -tvf ftp38.war`
   * `cp ftp38.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp38.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp38/
...
# Workshop material - Day #4 - Setup Jira, use it to run a complete sprint
  ...
  * Login to Jira and select your project FTP38.
